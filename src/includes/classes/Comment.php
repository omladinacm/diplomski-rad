...
class Comment
{
    ...
    public function create(): string
    {
        ...
        if ($numResponses > 0) {
            $viewRepliesText = "<span class='repliesSection viewReplies' onclick='getReplies($commentId, this, $videoId)'>
    ...
    public function getNumberOfReplies(): int
    {
        ...
        $query = $this->con->prepare("SELECT count(*) FROM comments WHERE responseToComment = :responseToComment");
        $query->bindParam(":responseToComment", $id);
        $query->execute();
    ...
    public function getReplies(): string
    {
        ...
        $query = $this->con->prepare("SELECT * FROM comments WHERE responseToComment = :commentId ORDER BY datePosted");
        $query->bindParam(":commentId", $commentId);
        $query->execute();
